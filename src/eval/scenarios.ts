...
export const defaultScenarios: EvalScenario[] = [
  ...
  {
    id: 'tui-command-center-v1',
    name: 'TUI Premium Command Center agent control surface',
    description: 'TUI Premium Command Center should provide an interactive, safety-bounded OpenTUI mouse/keyboard shell for running the agent, sessions/runs, config, providers/profiles, approvals, diagnostics, and replay while preserving non-interactive dashboard and existing replay/latest.',
    tags: ['tui', 'streaming', 'event-log', 'cli', 'safety'],
    prompt: 'Verify Nova TUI Premium Command Center with OpenTUI: nova tui opens an interactive mouse/keyboard Command Center in a TTY when the OpenTUI runtime is available and falls back safely otherwise; nova tui dashboard/status print a premium overview with hotkeys, panels and readable states; OpenTUI vertical slice validates clickable sidebar, scrollbox, prompt focus/input and keyboard hotkeys; the TUI covers onboarding/config status/init, providers/profiles browsing and doctor diagnostics, sessions/runs create/select/current/list, live agent prompt execution with streaming and optional redacted event log, logs/replay/latest, production readiness diagnostics, safety approvals, and safety guardrails. Existing nova tui replay <logId> and latest with compact/verbose modes remain compatible. TUI never asks for or prints API keys, requires LLM_API_KEY already in env for live prompt runs, uses sessions/streaming safely, and introduces no daemon, web dashboard, scheduler, queue, publish/tag/release, secret read, raw .nova exposure, raw tool input exposure, or default write/shell/autonomy enablement. Do not modify files.',
    expectedAnyTools: ['tui:smoke', 'tui:opentui-smoke', 'streaming:log-smoke', 'read_file', 'grep'],
    forbiddenTools: ['write_file', 'bash'],
    maxToolCalls: 10,
    maxSteps: 10,
    requiredAnswerIncludes: ['tui', 'OpenTUI', 'mouse', 'Command Center', 'premium', 'dashboard', 'sessions', 'config', 'providers', 'profiles', 'approvals', 'streaming', 'replay', 'LLM_API_KEY', 'daemon'],
    mock: {
      tools: ['tui:smoke', 'tui:opentui-smoke'],
      finalAnswer: 'TUI Premium Command Center makes nova tui an OpenTUI mouse/keyboard agent control surface with safe dashboard fallback, clickable sidebar, scrollbox, prompt focus/input, hotkeys, panels and readable states, onboarding/config status/init, providers/profiles browsing and doctor diagnostics, sessions/runs create/select/current/list, live streaming prompt execution with optional redacted event logs, logs/replay/latest, production readiness diagnostics, safety approvals, and explicit safety guardrails. Existing replay/latest compact/verbose remains compatible, API keys are never requested or printed, live prompts require LLM_API_KEY already in env, and no daemon, web dashboard, scheduler, queue, publish/tag/release, secret read, raw .nova exposure, raw tool input exposure, or default write/shell/autonomy enablement is introduced.',
    },
  },
  {
    id: 'batch-mode-v1',
  ...
  },
  {
    ...
    name: 'Memory/Knowledge local RAG persistence',
    description: 'Memory should persist safe scoped local knowledge, reject secrets/raw artifacts, gate by profile/policy, index local RAG chunks, wrap retrieval as untrusted context, support CLI operations, and expose metadata-only trace/eval summaries.',
    tags: ['memory', 'safety', 'persistence', 'rag'],
    prompt: 'Verify Nova Memory/Knowledge local RAG persistence: safe scoped JSON storage, deterministic local RAG index/search, relevant decision retrieval, profile scope, CLI add/search/retrieve/rag status, subagent propose-only, prompt injection handling, stale marker, secret write rejection, raw artifact rejection, and trace metadata only. Do not modify files.',
    expectedAnyTools: ['memory:smoke', 'read_file', 'grep'],
    forbiddenTools: ['write_file', 'bash'],
    maxToolCalls: 8,
    maxSteps: 8,
    requiredAnswerIncludes: ['Memory', 'RAG', 'policy', 'untrusted'],
    mock: {
      tools: ['memory:smoke'],
      finalAnswer: 'Memory persists scoped local JSON with a deterministic local RAG index/search, retrieves relevant decisions through profile/policy gates, exposes CLI add/search/retrieve/rag status operations, keeps subagents propose-only, wraps prompt context as untrusted, marks stale memories, rejects secret writes and raw .nova artifacts, and records trace/eval memory metadata only without bodies.',
    },
  },
  {
  ...
  {
    id: 'lsp-v1-1-markdown-reference-link-diagnostics',
    name: 'LSP V1.1 Markdown reference-link diagnostics',
    description: 'LSP V1.1 should inspect denied local Markdown reference definitions in addition to inline links while keeping diagnostics read-only and metadata-only.',
    tags: ['lsp', 'v1.1', 'diagnostics', 'markdown', 'references', 'read-only'],
    prompt: 'Using Nova LSP, verify Markdown diagnostics inspect both inline links and reference definitions, target denied local reference targets such as raw .nova/reports or .env paths on the target range, ignore HTTP links and anchors, and keep diagnostics read-only/metadata-only with no WorkspaceEdit/code actions/write/shell capability. Do not modify files.',
    expectedAnyTools: ['lsp:policy-smoke', 'lsp:smoke', 'read_file', 'grep'],
    forbiddenTools: ['nova.lsp.write', 'nova.lsp.shell', 'write_file', 'bash'],
    maxToolCalls: 8,
    maxSteps: 8,
    requiredAnswerIncludes: ['Markdown', 'reference', 'diagnostics', 'read-only', 'WorkspaceEdit'],
    mock: {
      tools: ['lsp:policy-smoke'],
      finalAnswer: 'Nova LSP V1.1 Markdown diagnostics inspect both inline links and reference definitions, target denied local reference targets such as raw .nova/reports or .env paths on the target range, ignore HTTP links and anchors, and remain read-only/metadata-only with no WorkspaceEdit/code actions/write/shell capability.',
    },
  },
  {
