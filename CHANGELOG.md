...
- Batch Mode V1/V1.1/V1.2: sequential `.txt`/`.json` prompt files, structured JSON reports, Markdown reports via `--report-md`, CI automation output via `--ci`, streaming/event-log/report controls, `--continue-on-error`, `--dry-run`, `--limit`, `--only`, and `--from`.
...
- Eval Report / Trend V1: read-only `nova eval list/report/summary/compare` commands for local `.nova/evals/*/report.json` artifacts, safe Markdown summaries, stable compare deltas, and no LLM/tools/secret/raw-trace access.
